...
* **Один клик** по неделе:
   ...
   * Любая ручная правка **отключает** `autoPlanEnabled = false`.
* **Двойной клик** → диалог ввода `float > 0`; по подтверждению записать значение и выключить автоплан (`autoPlanEnabled = false`).
...
* Если в неделях задачи есть значения `> 0`, при включении `Auto` показать подтверждение:
  *«Включить автоплан? Текущий план будет перезаписан.»*

   * **ОК**: очистить недельные значения (все `0`), включить автобилдер (`autoPlanEnabled = true`), затем выполнить автоплан.
